...
def train(fabric, model, optimizer, train_dataloader, val_dataloader, max_epochs=20):
    for epoch in range(max_epochs):
        train_loop(fabric, model, optimizer, train_dataloader, epoch)
        val_loss = validate(fabric, model,train_dataloader)
        fabric.print(f"Val Loss : {val_loss.item():.4f}")

def train_loop(fabric, model, optimizer, train_dataloader, epoch):
